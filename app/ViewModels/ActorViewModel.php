...
class ActorViewModel extends ViewModel
{
    ...
    public function actor()
    {
        return collect($this->actor)->merge([
            ...
            'profile_path' => $this->actor['profile_path']
                ...
                : 'https://ui-avatars.com/api/?size=350&background=random&name='.$this->actor['name'],
        ]);
    }
            ...
            if (isset($movie['title'])) {
              ...
              return collect($movie)->merge([
                  'poster_path' => $movie['poster_path']
                      ...
                      : 'https://ui-avatars.com/api/?size=185&background=random&name='.$title,
                  'title' => $title,
    ...
    {
        $castWork = collect($this->credits)->get('cast');
        return collect($castWork)->map(function ($work) {
            if (isset($work['release_date'])){
                $releaseDate = $work['release_date'];
            } elseif (isset($work['first_air_date'])){
                $releaseDate = $work['first_air_date'];
            } else {
                $releaseDate = '';
            }
            if (isset($work['title'])){
            ...
            return collect($work)->merge([
                'release_date' => $releaseDate,
                'release_year' => isset($releaseDate) ? Carbon::parse($releaseDate)->format('Y') : 'Future',
                'title' => $title,
                'character' => isset($work['character']) ? $work['character'] : '',
            ]);
